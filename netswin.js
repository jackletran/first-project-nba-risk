class Game {
  ...
  randomStateAssignment() {
    ...
    for (let i = 0; i < statesLength - 1; i++) {
      this.player2.stateInfo.unitsInThatState[`${shuffledStates[i]}`] += 100;
    }

    for (let c = statesLength - 1; c < statesLength; c++) {
      this.player1.stateInfo.unitsInThatState[`${shuffledStates[c]}`] += 1;
    }
  }
